...
def get_point_chart(data):
    ...
    chart = alt.Chart(data).mark_circle().encode(
        ...
        #color = alt.Color('type:', scale = alt.Scale(domain=['HR', 'qor15','step','adjustChart'], range=['red', 'blue','green','white']))
        color = alt.Color(':Q', scale = alt.Scale(scheme='greenblue'))
    )
...
if with_zero == True:
    with open(os.path.join('.','df_data','individual_activity_visualization_without_zero.pkl'),'rb') as f:
        chart_df = pickle.load(f)
        chart_df = chart_df.loc[(chart_df['patient_id'] == patient_id) ,:]
else:
    with open(os.path.join('.','df_data','individual_activity_visualization_with_zero.pkl'),'rb') as f:
        chart_df = pickle.load(f)
        chart_df = chart_df.loc[(chart_df['patient_id'] == patient_id) ,:]
