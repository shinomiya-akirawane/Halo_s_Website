...
def get_line_chart(data,y_value:str):
    ...
    chart = alt.Chart(data).mark_line(point=alt.OverlayMarkDef(filled=False, fill="white")).encode(
        alt.X('start_datetime:T',title = 'Date'),
...
standard_HR = st.slider('Please set up standard HR: ',50,150)
...
horizontal_line = alt.Chart(pd.DataFrame({'y': [standard_HR]})).mark_rule(color='red').encode(
    y='y'
)
chart = alt.layer(chart,horizontal_line)
